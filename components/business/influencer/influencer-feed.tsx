...
import { getBusinessContent, influencerContent } from "@/lib/mock-data/business-content"
...
const influencerConfig: BusinessConfig = {
  model: "influencer",
  name: "Camila Torres",
...
const influencerStories: BusinessStory[] = [
...
function mapVideoPosts(videos: BusinessPost[]): BusinessPost[] {
...
function mapSocialPosts(): BusinessPost[] {
  return influencerContent.socialPosts.map((post) => ({
    id: post.id,
    type: "social" as const,
    title: post.content.slice(0, 48),
...
export function InfluencerFeed() {
  ...
  useEffect(() => {
    const anyDrawerOpen = linksDrawerOpen || mediaKitDrawerOpen
  ...
  }, [linksDrawerOpen, mediaKitDrawerOpen, setComposerMode, setComposerOffsetClassName])
  ...
  const handleStoryClick = (story: BusinessStory) => {
    if (story.name === "Links") {
      setLinksDrawerOpen(true)
      return
    }
    if (story.name === "Collabs") {
      setMediaKitDrawerOpen(true)
    }
  }
  ...
  const sections: BusinessSection[] = [
    ...
    },
    {
      ...
      customContent: (
        <div className="space-y-3">
          ...
          <button
            onClick={() => setMediaKitDrawerOpen(true)}
            className="w-full p-3 text-sm text-accent hover:text-accent/80 transition-colors"
          >
            Ver media kit comercial
          </button>
        </div>
      ),
    },
    ...
    },
    {
      ...
      type: "content",
      posts: mapVideoPosts(content.videos),
    },
    {
      id: "posts",
      title: "Posts Recentes",
      type: "content",
      posts: mapSocialPosts(),
    },
  ]
  ...
  return (
    <ConversationSelectionProvider value={conversationSelection}>
      <>
        ...
        <ActionDrawer
          isOpen={mediaKitDrawerOpen}
          onClose={() => setMediaKitDrawerOpen(false)}
